...
const ItemReporting = () => {
  ...
  // Handle AI auto-fill from voice — applies ALL detected fields
  const handleAIFill = (aiData) => {
    setFormData(prev => {
      const updated = { ...prev };
      // Always overwrite if parser found something
      if (aiData.title)       updated.title       = aiData.title;
      if (aiData.description) updated.description = aiData.description;
      if (aiData.category)    updated.category    = aiData.category;
      if (aiData.location)    updated.location    = aiData.location;
      // type: "lost" or "found"
      if (aiData.type === 'lost' || aiData.type === 'found') {
        updated.type = aiData.type;
      }
      // date: parser returns YYYY-MM-DDTHH:MM ready for datetime-local input
      if (aiData.date)        updated.date        = aiData.date;
      return updated;
    });
  };
